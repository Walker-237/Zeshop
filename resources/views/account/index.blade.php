    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        :root {
            ...
            --pale: #f5f5f7; --white: #ffffff;
            --muted: #6e6e73; --border: #d2d2d7; --green: #30d158;
            --sans: -apple-system, 'Inter', sans-serif;
        }
        body { font-family: var(--sans); background: var(--pale); color: var(--ink); }
        a { text-decoration: none; color: inherit; }
        .account-wrap { max-width: 900px; margin: 40px auto; padding: 0 24px 80px; }
        .account-header {
        ...
        .orders-card-header {
            ...
            font-weight: 600; font-size: 15px; background: var(--pale);
        }
        .order-row {
            display: flex; align-items: center; justify-content: space-between;
            padding: 16px 20px; border-bottom: 0.5px solid var(--border); gap: 12px;
        }
        ...
        .empty-orders { text-align: center; padding: 48px 20px; color: var(--muted); font-size: 14px; }
        .empty-orders a {
            display: inline-block; margin-top: 16px; background: var(--blue);
            color: #fff; padding: 12px 28px; border-radius: 30px; font-weight: 600;
        }
    </style>
</head>
<body>
...
<div class="account-wrap">
    ...
    @if(session('success'))
        <div style="background:#e8f5e9;border:1px solid #a5d6a7;border-radius:12px;padding:12px 16px;margin-bottom:20px;font-size:13px;color:#2e7d32;">
            ✅ {{ session('success') }}
        </div>
    @endif
